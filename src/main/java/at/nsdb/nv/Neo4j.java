...
//		long count = (long) result.columnAs( "count").next();
		...
		String cypherQ = String.format("MATCH (p {id: %d}), (q {id: %d}) " + "CREATE (p)-[:%s]->(q)", connection.a,
				connection.b, RelTypes.KNOWS);
...
//					"RETURN p", 3, 6, RelTypes.KNOWS);
			...
			String cypherQ = String.format("MATCH (p1:Person {id: %d}), (p2:Person {id: %d}), "
					+ "path = shortestPath( (p1)-[*]-(p2)) " + "WHERE length( path) > 1 " + "RETURN path", 2, 40);

			Utils.logging(cypherQ);

			try (Result result = tx.execute(cypherQ)) {
				Utils.logging(String.format("Query: #columns=%d, at least one row=%s", result.columns().size(),
						result.hasNext()));
				...
				Utils.logging(result.resultAsString());
...
//					it.forEachRemaining( node -> System.out.println( node.getProperty( "path")));
	...
	public void printStatus() {
		...
		Utils.logging(
				String.format("%d (%d) relations found, for example:", connections.size(), this.getNumbRelations()));
		for (int i = 1; i <= 3; i++) {
